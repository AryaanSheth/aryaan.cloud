...
type View = { kind: "portfolio" } | { kind: "blog-list" } | { kind: "blog-post"; slug: string };

function hashToView(hash: string): View {
  if (hash === "#blog") return { kind: "blog-list" };
  ...
  return (
    <div className="page">
      <aside className="toc" aria-label="Table of contents">
        ...
        <nav>
          <ul className="toc__list">
            <li><a href="#index" onClick={() => navTo("#index")}><span className="num">00</span><span>Index</span></a></li>
      ...
      <article className="main">
        ...
        {view.kind === "blog-list" && (
          <BlogList posts={posts} onSelect={slug => navTo(`#blog/${slug}`)} />
        )}

        {view.kind === "blog-post" && (
          <BlogPost slug={view.slug} onBack={() => navTo("#blog")} />
        )}
      </article>
    </div>
  );
